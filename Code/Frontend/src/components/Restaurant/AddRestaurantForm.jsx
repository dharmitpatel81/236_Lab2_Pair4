...
const AddRestaurantForm = ({ onSuccess, onCancel }) => {
    const restaurantId = useSelector((state) => {
        return state.auth?.restaurant?.id;
    });
    ...
    const [formData, setFormData] = useState({
        ...
        ratings: 0,
        image_url: null,
        restaurant_id: null  // Initialize as null
    });
    ...
    // Update formData when restaurantId changes
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        console.log('Form submission - Current restaurantId:', restaurantId);
        ...
        // Create a new object with the current restaurantId to ensure it's included
        const dataToSubmit = {
            ...formData,
            restaurant_id: restaurantId // Explicitly set the restaurant_id from Redux
        };

        console.log('Data being submitted:', dataToSubmit);

        if (!dataToSubmit.restaurant_id) {
            setError('Restaurant ID is required. Please try refreshing the page.');
            setLoading(false);
            return;
        }
